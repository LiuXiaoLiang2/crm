...
<SCRIPT language=javascript>
	...
	$(function(){
		...
		$.post(url,params,function(result){
			...
			//实现回显功能
			//Action对象还是在栈顶的.从栈顶中获取
			var valueId = "${customer.cust_id}";
			//alert(valueId)
			//使用juery选中
			$("option[value='"+valueId+"']").attr("selected",true)
		},"json")
	})
	
	
	/* 用于显示客户所属于的联系人 */
	function findCust(obj,link_id,cust_id){
		//alert(obj.name + "--" + cust_id );
		if(obj.name == "关闭"){
			$("#"+link_id).html("");
			obj.name = "打开";
		} else {
			//发送Ajax请求查询
			var url = "${pageContext.request.contextPath }/customer_findByCritera.action";
			var param = {"cust_id":cust_id};
			$.post(url,param,function(result){
				//alert(JSON.stringify(result))
				$("#"+link_id).html(result.cust_name);
				obj.name = "关闭";
			},"json");
		}
	}
										...
										<TABLE id=grid
											...
											<TBODY>
												...
												<TR
													...
													&nbsp;&nbsp;
													<a href="javascript:void(0);" name="打开" onclick="findCust(this,${linkman.lkm_id},${linkman.customer.cust_id })">查看所属客户</a>
													<div id="${linkman.lkm_id}">
